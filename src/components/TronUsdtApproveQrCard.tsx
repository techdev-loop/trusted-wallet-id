...
export function TronUsdtApproveQrCard() {
  ...
  const openedForTronUsdtQr = searchParams.get("tronUsdt") === "1";
  ...
  const dashboardApproveUrl = useMemo(() => {
    if (typeof window === "undefined") return "";
    return buildDashboardTronUsdtApproveUrl({
  ...
  const trustWalletScanUrl = useMemo(
    () => (dashboardApproveUrl ? buildTrustWalletTronDappOpenUrl(dashboardApproveUrl) : ""),
    [dashboardApproveUrl]
  );
  ...
  useEffect(() => {
    if (!openedForTronUsdtQr || !spender || trustAutoConnectAttempted.current) return;
          ...
          <>
            ...
            {trustWalletScanUrl ? (
              <div className="flex flex-col items-center gap-3">
                ...
                {showPlainWalletQr && dashboardApproveUrl ? (
                  <div className="flex flex-col items-center gap-2 pt-2 border-t border-border w-full">
                    <div className="rounded-xl bg-white p-3">
                      <QRCode value={dashboardApproveUrl} size={200} level="M" />
                    </div>
                    <Button
                      type="button"
                      variant="ghost"
                      size="sm"
                      className="gap-2"
                      onClick={() => copyLink(dashboardApproveUrl, "Dashboard link")}
                    >
                      <Copy className="h-4 w-4" />
                      Copy dashboard link
                    </Button>
